...
var history = new Vue({
  ...
  methods: {
    fetchGroups: function fetchGroups() {
      ...
      this.apiCall('/groups').then(function (res) {
    ...
    fetchPosts: function fetchPosts() {
      ...
      this.apiCall('/posts').then(function (res) {
        _this2.posts = res.data;
      }).catch(function (err) {
        _this2.errors.push(err);
      });
    },
    apiCall: function apiCall(path) {
    ...
    filterPosts: function filterPosts() {
      ...
      var time = this.search.time;
      var group = this.search.group;

      return this.posts.filter(function (post) {
        var matched = true;
        if (text) matched = matched && _this3.matchText(post, text);
        // if (time) matched = matched && this.matchTime(post.time, time);
        if (group != -1) matched = matched && post.group_info.id === group;
        return matched;
      });
    },
    matchTime: function matchTime(t1, t2) {
      var dt1 = new Date(t1);
      var dt2 = new Date(t2);
      return dt1.getFullYear() === dt2.getFullYear() && dt1.getMonth() === dt2.getMonth() && dt1.getDate() === dt2.getDate();
    },
    matchText: function matchText(post, text) {
      return post.group_info.name === text || post.post_text === text || post.account_info.type === text;
    },
  ...
  computed: {
    showablePosts: function showablePosts() {
      var posts = this.filterPosts() || [];
      var count = posts.length;
      var start = this.paginagtion.perPage * (this.paginagtion.current - 1) + 1;
      var end = start + this.paginagtion.perPage;

      var postsToShow = [];
      for (var i = start; i < count && i <= end; i++) {
